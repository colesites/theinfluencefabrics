...
type CollectionPageProps = {
  searchParams: Promise<{ category?: string; color?: string; page?: string; q?: string }>;
}
...
export default async function CollectionPage({ searchParams }: CollectionPageProps) {
  ...
  const [allProducts, products, totalCount, availableColors] = await Promise.all([
    getProducts(),
    getPaginatedProducts({ category, color, query, page: currentPage, limit: ITEMS_PER_PAGE }),
    getTotalProductCount(category, color, query),
    getAvailableColors()
  ]);

  const uniqueCollections = Array.from(new Set(allProducts.map(p => p.collection))).filter(Boolean);
  ...
  const getUrl = (newParams: { category?: string; color?: string; page?: number | null; q?: string | null }) => {
    ...
    if (newParams.category !== undefined) {
      if (newParams.category && newParams.category !== "All Archives") params.set('category', newParams.category);
    ...
    if (newParams.page !== undefined) {
      if (newParams.page && newParams.page > 1) params.set('page', String(newParams.page));
    ...
    return params.toString() ? `/collection?${params.toString()}` : "/collection";
  };
      ...
      <header className="mb-14 sm:mb-20">
        ...
        <div className="mt-8 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <p className="max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-lg">
            A curated selection of authentic wax prints celebrating architectural geometry and
            heritage patterns. Each textile is a narrative of tradition reimagined for the modern
            silhouette.
          </p>
          <div className="flex items-center gap-4">
            <span className="text-xs uppercase tracking-widest text-muted-foreground">{totalCount} Pieces Found</span>
            {isSearchActive && (
              <Link 
                href="/collection" 
              ...
              >
                ...
                <span className="text-[10px] uppercase font-black tracking-widest">Results for: {query}</span>
        ...
        <aside className="space-y-12 lg:sticky lg:top-32 lg:h-fit">
          ...
          <div className="border-t border-black/10 pt-8">
            <div className="flex items-center justify-between">
              <p className="editorial-kicker text-black/70">Filter by palette</p>
              {color && (
                <Link href={getUrl({ color: "" })} className="text-[9px] uppercase tracking-widest opacity-50 hover:opacity-100">Clear</Link>
              )}
            </div>
            <div className="mt-5 flex flex-wrap gap-3">
              {availableColors.map((swatch) => (
                <Link
                  ...
                  style={{
                    ...
                    outline: color === swatch ? "2px solid #000" : "1px solid rgba(0,0,0,0.1)",
                    outlineOffset: "2px"
                  }}
                >
              ...
              {availableColors.length === 0 && (
                <p className="text-[10px] text-muted-foreground italic">No seasonal colors found.</p>
              )}
            </div>
          </div>
        </aside>
          ...
          <div className="grid gap-x-10 gap-y-16 md:grid-cols-2 lg:gap-y-24">
            {products.length === 0 && (
              <div className="md:col-span-2 py-20 bg-surface-dim flex flex-col items-center justify-center border border-dashed border-black/10">
                <p className="text-muted-foreground font-serif italic">No textiles found matching your search.</p>
                <Link href="/collection" className="mt-4 text-xs uppercase tracking-widest text-primary font-bold">Clear All Filters</Link>
              </div>
            )}
            {products.map((product, index) => (
              <Card
                key={product._id}
                className={index % 2 === 1 ? "bg-transparent md:translate-y-10 border-none shadow-none" : "bg-transparent border-none shadow-none"}
              >
                <Link
                ...
                >
                  <Image
                    src={product.image || "https://lh3.googleusercontent.com/aida-public/placeholder-image"}
                    alt={product.name}
                    fill
                ...
                <CardContent className="px-0 pb-0 pt-7">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="font-serif text-3xl font-black">₦{product.price.toLocaleString("en-NG")}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
          ...
          {totalPages > 1 && (
            <PaginationRoot className="mt-32">
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious 
                    href={currentPage > 1 ? getUrl({ page: currentPage - 1 }) : "#"} 
                ...
                <PaginationItem>
                  <PaginationNext 
                    href={currentPage < totalPages ? getUrl({ page: currentPage + 1 }) : "#"} 
